...
class Main
  ...
  def create_wagon
    ...
    puts "
         0 - Грузовой
         1 - Пассажирский"
    type = %w[cargo passenger][gets.chomp.to_i]
  ...
  def train_moving
    ...
    if num == '1'
      puts "Поезд #{train.number} переместился со станции #{train.previous_station.inspect}
            на станцию #{train.current_station}"
    end
    return unless num == '2'

    puts "Поезд #{train.number} переместился со станции #{train.current_station}
          на станцию #{train.previous_station.inspect}"
  end
  ...
  def show_wagons
    ...
    station.trains.each do |train|
      train.show_wagon do |wagon|
        puts "#{rand(100..999)}, #{wagon.type} #{if wagon.type == :passenger
              [wagon.free_place, wagon.used_place].join(' ')
              else
              [wagon.free_place, wagon.used_place].join(' ')
              end}"
      end
